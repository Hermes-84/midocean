...
def _bucket_cm2_list(cm2_csv: str) -> str:
    ...
    for part in str(cm2_csv).split(","):
        ...
        out.append(b)
    # dedup mantenendo ordine per valore desc
...
def main():
    ...
    # decimali UE anche per carton length/width/height/weight/volume
    carton_cols_eu = [
    ...
    # --- REORDER: inserisci le nuove colonne vicino a quelle richieste ----------
    cols = list(df.columns)

    # vicino a SKU metti l'EAN
    try:
        i = cols.index("products__product__product_number")
        cols.insert(i+1, cols.pop(cols.index("products__product__ean")))
    except ValueError:
        pass

    # vicino a manipulation metti manipulation_man
    try:
        i = cols.index("products__product__manipulation")
        cols.insert(i+1, cols.pop(cols.index("products__product__manipulation_man")))
    except ValueError:
        pass

    # vicino a max_print_area metti max_print_area_rounded
    try:
        i = cols.index("max_print_area")
        cols.insert(i+1, cols.pop(cols.index("max_print_area_rounded")))
    except ValueError:
        pass
